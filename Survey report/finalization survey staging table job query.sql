...
CREATE PROCEDURE USP_finalizationSurveyStg
AS
BEGIN

IF EXISTS (select 1 from sys.objects where name='StgTbl_SurveyRpt_149' and type='u')
DROP TABLE StgTbl_SurveyRpt_149;

IF EXISTS (select 1 from sys.objects where name='StgTbl_SurveyRpt_finalization' and type='u')
DROP TABLE StgTbl_SurveyRpt_finalization;
		...
		WHERE sc.Campaign_id=149
		

--StgTbl_SurveyRpt_149
...
FROM StgTbl_SurveyRpt_Finalization
INSERT INTO #qns
(qn)
...
BEGIN
	...
	FROM #names where id=@mincnt
		 ...
		 BEGIN
				...
				SET @answer_n = 0
				SET @squery= '
				 SELECT @answer_n = ISNULL(answer,0) from StgTbl_SurveyRpt_Finalization
				...
				SET @dquery='
				 UPDATE StgTbl_SurveyRpt_Finalization
