...
CREATE PROCEDURE USP_HomeStudyServicesSurveyStg
AS
BEGIN

IF EXISTS (select 1 from sys.objects where name='StgTbl_SurveyRpt_111' and type='u')
DROP TABLE StgTbl_SurveyRpt_111;

IF EXISTS (select 1 from sys.objects where name='StgTbl_SurveyRpt_HomeStudyServices' and type='u')
DROP TABLE StgTbl_SurveyRpt_HomeStudyServices;
		...
		WHERE sc.Campaign_id=111
		

--StgTbl_SurveyRpt_111
...
BEGIN
	...
	FROM #names where id=@mincnt
		 ...
		 BEGIN
				...
				SET @answer_n = 0
				SET @squery= '
				 SELECT @answer_n = ISNULL(answer,0) from StgTbl_SurveyRpt_HomeStudyServices
