...
CREATE PROCEDURE USP_FamilyForeverEvaluationSurveyStg
AS
BEGIN

IF EXISTS (select 1 from sys.objects where name='StgTbl_SurveyRpt_110' and type='u')
DROP TABLE StgTbl_SurveyRpt_110;

IF EXISTS (select 1 from sys.objects where name='StgTbl_SurveyRpt_FamilyForeverEvaluation' and type='u')
DROP TABLE StgTbl_SurveyRpt_FamilyForeverEvaluation;
		...
		WHERE sc.Campaign_id=110
		

--StgTbl_SurveyRpt_110
...
FROM StgTbl_SurveyRpt_FamilyForeverEvaluation
INSERT INTO #qns
(qn)
VALUES('q1') 
DECLARE @mincnt		INT 
...
BEGIN
	...
	FROM #names where id=@mincnt
		 ...
		 BEGIN
				...
				set @answer_n=0
				SET @squery= '
				 SELECT @answer_n = isnull(answer,0) from StgTbl_SurveyRpt_FamilyForeverEvaluation
				...
				SET @dquery='
				 UPDATE StgTbl_SurveyRpt_FamilyForeverEvaluation
