...
BEGIN
		...
		BEGIN
				...
				WHERE 
								...
								AND (ua.agency_id = (select agency_id from user_accounts where user_id=@user_id ) 
								OR up.user_id in ( select user_id from user_accounts 
								WHERE case_worker_parent_user_id =(select case_worker_parent_user_id from user_accounts 
		...
		BEGIN
				...
				INSERT INTO #documents		(upload_id
					...
					WHERE
						...
						OR up.user_id in (@user_id)
						OR up.user_id in (@user_id)  
						OR up.group_id in (SELECT group_id FROM upload where user_id = @user_id)
							)  
		...
		WHERE (id>=@startrow AND id<=@endrow)
				...
				AND ((last_name like case when(isnull(@user_name,'')!='') then ('%' + @user_name + '%') else last_name end))
