...
select f.tablename,f.fieldname,f.fieldtype,f.lkptable,m.fielddata into #temp_medinfo
...
update #temp_medinfo 
set MedGenPullDownId=fielddata
,MedGenNote='Pulldown'
where lkptable >''
...
--if (select lkptable from #temp_medinfo) is not null

--begin

--insert into #temp_medinfo(MedGenPullDownId,MedGenNote)
--values((select fielddata from #temp_medinfo),'Pulldown')


--end

--Else If (select fieldtype from #temp_medinfo) = 'date' or (select fieldtype from #temp_medinfo) = 'Datetime'
--begin
--		insert into #temp_medinfo(MedGenDate)
--		values((select fielddata from #temp_medinfo))
--end
