...
--EXEC GetCasenoteData_test 164, 'Case Note *', 86395

CREATE PROCEDURE [dbo].[GetCasenoteData_test]	 
     @cAccountkey INT
	...
	ELSE
		BEGIN
			SET @strQuery = 'SELECT  DISTINCT 
									...
									JOIN' + @strDBName + '.dbo.lkpSecureCaseNoteTypes CN on CN.ContactActivityId=A.ContactActivityId
									JOIN' + @strDBName + '.dbo.lkpSecureCaseNoteSubTypes CNS on CNS.ContactActivityId=A.ContactActivityId
