Text
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
exec CaseNote_Details_new -93633,25, 100 ,0,NULL,345007,257,'','DESC',945

ALTER PROCEDURE [dbo].[CaseNote_Details_new]
      @Connid VARCHAR(200)
     ,@UserId INT
     ,@NoofRows INT
     ,@Start_id INT
     ,@filet_Value VARCHAR(MAX)
     ,@ConnectionId Int = 0
     ,@loginUserId Int = 0
...
IF @Noofrows = 0 
  BEGIN 
	SELECT  @End_id = MAX(Row_No) FROM #CaseNote_Details
	SET @Start_id = 0
  END 
ELSE
BEGIN
	SET @End_id = @Start_id + @Noofrows  
END			


ALTER TABLE #CaseNote_Details
ADD issecure int
ALTER TABLE #CaseNote_Details
ADD isread int


--INSERT INTO #CaseNote_Details(issecure)
--SELECT l.issecure from lkpacttype l
--RIGHT JOIN #CaseNote_Details c on l.ActTypeId=c.acttypeid

--INSERT INTO #CaseNote_Details(isread)
--SELECT s.isread from securitysettings s
--RIGHT JOIN #CaseNote_Details c on s.sec_casenotetypeID=c.acttypeid and s.sec_groupID=@groupID and s.sec_cwID=@loginUserId

;
WITH issecure_tbl as(SELECT l.issecure,l.acttypeid from lkpacttype l
JOIN #CaseNote_Details c on l.ActTypeId=c.acttypeid)

UPDATE c SET c.issecure = i.issecure
FROM #CaseNote_Details AS c JOIN issecure_tbl AS i ON c.acttypeid = i.acttypeid

;
WITH isread_tbl as(SELECT s.isread,s.sec_casenotetypeID,s.sec_groupID,s.sec_cwID from securitysettings s
JOIN #CaseNote_Details cc on s.sec_casenotetypeID=cc.acttypeid and s.sec_groupID=@groupID and s.sec_cwID=@loginUserId)

UPDATE cc SET cc.isread = r.isread
FROM #CaseNote_Details AS cc JOIN isread_tbl AS r ON r.sec_casenotetypeID=cc.acttypeid and r.sec_groupID=@groupID and r.sec_cwID=@loginUserId
