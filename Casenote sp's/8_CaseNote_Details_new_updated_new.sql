...
IF exists (select 1 from sys.objects where name='CaseNote_Details_new' and type='p')
DROP PROCEDURE CaseNote_Details_new;

GO

CREATE PROCEDURE [dbo].[CaseNote_Details_new]
      @Connid VARCHAR(200)
     ,@UserId INT
     ,@NoofRows INT
     ,@Start_id INT
...
WHERE Row_No > @Start_id
	  AND Row_No <= @END_id  
	  AND (
		   (@user_type='agency')
		   OR (@user_type='agency_user' AND isnull(@setting,'no')='no' AND isnull(issecure,0)=0)
		   OR (@user_type='agency_user' AND @setting='yes' AND isnull(isread,1)=1)
		   )
