...
(SELECT ROW_NUMBER() OVER(PARTITION BY c.contactid ORDER BY R1.RelationshipID DESC)AS reccount
 ,R1.RelationshipID,RID.RID_Value,R1.ContactAIRSId,c.contactid
 FROM Rel_Relationship R1
 INNER JOIN RelationshipInfoData RID on R1.RelationshipID = RID.RelationshipID
 INNER JOIN Contact c ON R1.ContactAIRSId = (c.ContactId * -1)
 WHERE R1.RelationshipTypeId = -10
 AND RID.RID_ItemName = 'Annual_Salary')

UPDATE R
SET	r.Salary=s.RID_Value 
FROM RptContact R	
