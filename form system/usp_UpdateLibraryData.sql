...
BEGIN
 ...
 Insert Into #Temp1 (InputFieldname,InputFieldvalue )
 SELECT [column].value('fn:local-name(.)', 'NVARCHAR(max)') as 'InputFieldname'
  ,[column].value('.', 'NVARCHAR(max)') as 'InputFieldvalue'
  FROM @xmldata_input.nodes('//.') as [table]([column])
  WHERE [column].value('fn:local-name(.)','nvarchar(max)')<>'';
  ...
  Select  DENSE_RANK() Over (Order By inputfieldname) RNK,* Into #xmldata_rank From #Temp1 Where inputfieldname IN 
 ('scope','program_id','connection_id','is_system','name') Order By ID

  --select * from #xmldata_rank

 SELECT DENSE_RANK() Over (Partition By RNK Order By ID) GRPNO,* Into #Resulttemp FROM #xmldata_rank  Order By ID
 ...
 --Select * From #Resulttemp
  ...
  ,is_system 
  ,name 
   FROM #Resulttemp AS SourceTable
   PIVOT
  (
  max(inputfieldvalue)
 ...
 select * into #inputfields from #xmldata_input
 where inputfieldname in ('name','value')

 alter table #inputfields
 add id int identity(1,1)

 Select  DENSE_RANK() Over (Order By inputfieldname) RNK,* Into #xmldatarank From #inputfields Where inputfieldname IN 
 ('name','value') Order By id
 
 SELECT DENSE_RANK() Over (Partition By RNK Order By ID) GRPNO,* Into #Result_temp FROM #xmldatarank  Order By ID
 ...
 --Select * From #Result_temp
  ...
  ,name
  ,value
   FROM #Result_temp AS SourceTable
   PIVOT
  (
  max(inputfieldvalue)
  FOR inputfieldname IN 
