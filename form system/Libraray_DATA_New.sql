...
BEGIN  
		...
		ca.MailingAddress=0 And Not Exists(Select 1 From contactaddress cad Where cad.MailingAddress=1 And cad.contactid = @contactid)))	
		...
		ce.PrimaryEmail=0 And Not Exists(Select 1 From contactemail cem Where cem.PrimaryEmail=1 And cem.contactid = @contactid)))
		left join rolesqa.dbo.contactphone cp
		on c.contactid=cp.contactid and (cp.PrimaryPhone=1 Or (
		cp.PrimaryPhone=0 And Not Exists(Select 1 From contactphone cph Where cph.PrimaryPhone=1 And cph.contactid = @contactid)))
		...
		FROM  (SELECT Top 1 IsNull(cc.FName,'') as AP2Fname
					...
					ee.PrimaryEmail=0 And Not Exists(Select 1 From contactemail cema Where cema.PrimaryEmail=1 And cema.contactid = @contactid2)))
					left join contactphone pp
					on pp.contactid=@contactid2 and (pp.PrimaryPhone=1 Or (
					pp.PrimaryPhone=0 And Not Exists(Select 1 From contactphone cpho Where cpho.PrimaryPhone=1 And cpho.contactid = @contactid2)))
