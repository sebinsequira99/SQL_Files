...
--TRUNCATE TABLE library_systemdata_test
...
SELECT @start_row =Min(contactid) FROM mapat.dbo.Contact
SELECT @end_row= Max(contactid) FROM mapat.dbo.Contact
--Declare @I As Int =1
--Declare @J As Int = 2		
...
BEGIN  
		...
		FROM mapat.dbo.rel_relationship 
		...
		FROM mapat.dbo.contact c
		left join mapat.dbo.contactaddress ca
		on c.contactid=ca.contactid and (ca.MailingAddress=1 Or (
		ca.MailingAddress=0 And Not Exists(Select 1 From mapat.dbo.contactaddress cad 
		Where cad.MailingAddress=1 And cad.contactid = @contactid)))	
		left join mapat.dbo.lkpcounty lc
		on ca.countyid=lc.countyid
		left join mapat.dbo.lkpcountry cn
		on ca.countryid=cn.countryid
		left join mapat.dbo.contactemail ce
		on c.contactid=ce.contactid and (ce.PrimaryEmail=1 Or (
		ce.PrimaryEmail=0 And Not Exists(Select 1 From mapat.dbo.contactemail cem 
		Where cem.PrimaryEmail=1 And cem.contactid = @contactid)))
		left join mapat.dbo.contactphone cp
		on c.contactid=cp.contactid and (cp.PrimaryPhone=1 Or (
		cp.PrimaryPhone=0 And Not Exists(Select 1 From mapat.dbo.contactphone cph 
		...
		FROM  (SELECT Top 1 IsNull(cc.FName,'') as AP2Fname
					...
					FROM mapat.dbo.contact cc
					left join mapat.dbo.contactemail ee
					on ee.contactid=cc.contactid and (ee.PrimaryEmail=1 Or (
					ee.PrimaryEmail=0 And Not Exists(Select 1 From mapat.dbo.contactemail cema 
					Where cema.PrimaryEmail=1 And cema.contactid = @contactid2)))
					left join mapat.dbo.contactphone pp
					on pp.contactid=@contactid2 and (pp.PrimaryPhone=1 Or (
					pp.PrimaryPhone=0 And Not Exists(Select 1 From mapat.dbo.contactphone cpho 
					Where cpho.PrimaryPhone=1 And cpho.contactid = @contactid2)))
					where cc.contactid=@contactid2) B

		INSERT INTO library_systemdata_test
		(
		user_id
		,Contactid
		,connectionid
		...
		SELECT @start_row = MIN(contactid) FROM mapat.dbo.Contact Where contactid > @contactid
		--Set @I = @I + 1
END
--Select * From #tblXML
Drop Table #tblXML
--Select * From library_systemdata_test Order By contactid
SET NOCOUNT OFF