...
BEGIN  
		...
		SELECT @UserID As UserID
			  ...
			  ,(													
			   ...
			   (replace(
			   (replace
			   (replace(dbo.udf_CleanString(ct.CIListTemplateItemName,'')+ '_' + REPLACE(cast(cl.CIListTemplateItemId as varchar(4)), '-','_'),' ','_')
			   ,'?','.')
			   ),'/','_')
			   ) ,':','.')
			   ,'1st','First')
			   ...
			   THEN cast(cl.CIListNumber as nvarchar(100))
			   WHEN 'Date'
			   THEN convert(nvarchar(100),cl.CIListItemDate,120)
			   WHEN 'Text'
