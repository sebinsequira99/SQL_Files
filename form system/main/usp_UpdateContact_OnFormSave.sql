		...
		BEGIN
			...
			BEGIN
				...
				BEGIN
					...
					UPDATE dbo.Rel_Relationship SET DisplayName= dbo.udf_CoupleName(@ContactId * -1,0,1,'Unknown',0,0) 
					WHERE connid = @ContactId * -1
						
					SELECT @CoupleConnid = dbo.udf_coupleConnectionid(@ContactId * -1) 
					IF 	@CoupleConnid<0
						UPDATE dbo.Rel_Relationship SET DisplayName= dbo.udf_CoupleName(@CoupleConnid,0,1,'Unknown',0,0) 
						WHERE connid = @CoupleConnid			
					ELSE
						UPDATE dbo.Rel_Relationship SET DisplayName =  dbo.udf_CoupleName(@CoupleConnid,1,1,'Unknown',0,0) 
						WHERE connid = @CoupleConnid
				...
				IF(@fieldname='AP1BirthName')
				
				BEGIN
					select @birthname=fieldvalue FROM #XmlData where fieldname='AP1BirthName'
					set @birthname=replace(@birthname,' ',',')

					update contact
					set BirthFirstName=(SELECT value from udf_Split_into_rows(@birthname) where id=1)
					   ,BirthLastName=(SELECT value from udf_Split_into_rows(@birthname) where id=2)
					where contactid=@contactid
				END
			...
			BEGIN
				...
				BEGIN
					...
					SET address1=isnull((select fieldvalue from #XmlData_desc where fieldname='Add1'),'')
					   ,address2=isnull((select fieldvalue from #XmlData_desc where fieldname='Add2'),'')
					   ,city=isnull((select fieldvalue from #XmlData_desc where fieldname='City'),'')
					   ,state=isnull((select l.statename 
									  from #XmlData_desc x
									  join lkpstate l
									  on x.fieldvalue=l.stateid 
									  where x.fieldname='State'),'')
					   ,zipcode=isnull((select fieldvalue from #XmlData_desc where fieldname='Zip'),'')
					   ,country=isnull((select l.countrytext 
										from #XmlData_desc x
										join lkpcountry l
										on x.fieldvalue=l.countryid
										where x.fieldname='Country'),'')
					where connid=@connid
			...
			BEGIN
				...
				BEGIN
					...
					--UPDATE dbo.Rel_Relationship SET DisplayName= dbo.udf_CoupleName(@iContactId * -1,0,1,'Unknown',0,0) 
					--WHERE connid = @iContactId * -1
						
					--SELECT @CoupleConnid = dbo.udf_coupleConnectionid(@iContactId * -1) 
					--IF 	@CoupleConnid<0
					--	UPDATE dbo.Rel_Relationship SET DisplayName= dbo.udf_CoupleName(@CoupleConnid,0,1,'Unknown',0,0) 
					--	WHERE connid = @CoupleConnid			
					--ELSE
				...
				BEGIN
					...
					BEGIN
						...
						UPDATE dbo.Rel_Relationship SET DisplayName= dbo.udf_CoupleName(@ContactId2 * -1,0,1,'Unknown',0,0) 
						WHERE connid = @ContactId2 * -1
						
						SELECT @CoupleConnid = dbo.udf_coupleConnectionid(@ContactId2 * -1) 
						IF 	@CoupleConnid<0
							UPDATE dbo.Rel_Relationship SET DisplayName= dbo.udf_CoupleName(@CoupleConnid,0,1,'Unknown',0,0) 
							WHERE connid = @CoupleConnid			
						ELSE
					...
					IF(@fieldname='AP2BirthName')
				
					BEGIN
						select @birthname=fieldvalue FROM #XmlData where fieldname='AP2BirthName'
						set @birthname=replace(@birthname,' ',',')
