...
CREATE PROCEDURE [dbo].Usp_GetAllRelationships
...
BEGIN
 ...
 DECLARE @timestmp varchar(500) --current_timestamp



 SELECT @timestmp = current_timestamp 
 ...
 IF @RelTypeId=''  SET @RelTypeId ='-10000'
  ...
  IF NOT EXISTS(SELECT 1 FROM sys.Tables where name='tempRelType')
  	...
  	IF(@RelTypeId IS NOT NULL AND @RelTypeId !='-10000' )
	...
	PRINT @RelTypeId
			...
			INSERT  INTO tempRelType (Reltypeid,timestmp)   SELECT ArrayVal AS RelTypeId,@timestmp FROM ArrayListToTable(@RelTypeId) 			
  ...
  SET @SqlQry = @SqlQry + '
						   ...
						   , CONVERT(VARCHAR(10),(Select Top 1 r4.RelEndDate
						    ...
						    WHERE r.connid = ' + isnull(@ConnId1,'0') + '
							  ...
							  AND (('''+@RelTypeId+''' = ''-10000''  OR EXISTS(SELECT 1 FROM tempRelType tg where timestmp=  ''' +@timestmp + ''' AND ISNULL(r2.RelationshipTypeId,-10000)=  tg.RelTypeId )))'
  ...
  If @ConnId2 <> 0
   ...
   If @PgmConnectionId <> 0
			...
			SET @SqlQry = @SqlQry + '
								   ...
								   WHERE   ('''+@RelTypeId+'''  = ''-10000''  OR EXISTS(SELECT 1 FROM tempRelType tg where   timestmp=''' +@timestmp + ''' AND ISNULL(r.RelationshipTypeId,-10000)=  tg.RelTypeId )) 



								    AND r.Connectionid = ' + isnull(@PgmConnectionId,'0') 
 ...
 END
  ...
  DELETE FROM tempRelType where timestmp=@timestmp
