...
DELETE FROM pfcomm_mig_job.pdf_template_user 
...
set role_id = 5 
where role_id = 1 
and account_id=(select account_id from(select account_id from pfcomm_mig_job.account 
				where username=(select nickname from pfcomm_test.`Profiles` 
								where id=NEW.ProfileID)) as p);
...
UPDATE `pfcomm_mig_job`.`account` a, `pfcomm_test`.`Profiles` as p 
