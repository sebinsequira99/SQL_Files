...
AFTER INSERT ON pfcomm_mig_job.SyncLog
FOR EACH ROW
...
INSERT INTO pfcomm_mig_job.`profiles`(publish_status 
,nickname
,modified_at 
,last_name 
,last_login 
...
,dateofbirth 
,datereg 
FROM pfcomm_test.`Profiles`
WHERE id=NEW.ProfileID;

-- ACCOUNT TABLE

INSERT INTO pfcomm_mig_job.account(`name`
,emailid 
,username 
,`status`
,role_id 
...
,`date`
,val
from pfcomm_test.aqb_pc_members_blocks
...
,isDefault
,lastupdateddate
from pfcomm_test.pdf_template_user
...
insert into pfcomm_mig_job.bx_groups_main 
select * from pfcomm_test.bx_groups_main 
...
-- update pfcomm_mig_job.`profiles` as p
-- inner join pfcomm_test.`Profiles` as pp
-- on p.nickname=pp.nickname
...
FROM pfcomm_test.`Profiles` as p
...
inner join pfcomm_test.`Profiles` as p
...
,p.LastObjId
,p.AllowAlbumView 
from pfcomm_test.sys_albums as p
inner join pfcomm_test.`Profiles` as pp
on p.`owner`=pp.id
...
delete from pfcomm_mig_job.sys_albums_objects;
insert into pfcomm_mig_job.sys_albums_objects
select * from pfcomm_test.sys_albums_objects;
...
,r.ytStatusCheck
,r.ytChannelId
from pfcomm_test.RayVideoFiles as r
inner join pfcomm_test.`Profiles` as p
on r.owner=p.id
inner join pfcomm_mig_job.`profiles` as pn
...
,Featured
,`Status`
,`Hash` from pfcomm_test.bx_photos_main
where `owner`=NEW.ProfileID;

update pfcomm_mig_job.bx_photos_main b
inner join pfcomm_test.`Profiles` p
...
update pfcomm_mig_job.letter as l 
inner join pfcomm_test.letters_sort as ls 
...
delete from pfcomm_mig_job.waiting_new;
...
SELECT nickname,SUBSTRING_INDEX(SUBSTRING_INDEX(t.waiting, ',', n.n), ',', -1) value
  FROM pfcomm_test.Profiles t CROSS JOIN 
(
 ...
 and t.id=NEW.ProfileID;
 
delete from pfcomm_mig_job.religion_new;
...
delete from pfcomm_mig_job.ethnicity_new;
...
delete from pfcomm_mig_job.education_new;
...
delete from pfcomm_mig_job.adoptiontype_new;
...
delete from pfcomm_mig_job.childage_new;
...
delete from pfcomm_mig_job.childethnicity_new;
...
delete from pfcomm_mig_job.bfstatus_new;
...
delete from pfcomm_mig_job.faith_new;
...
delete from pfcomm_mig_job.ethnicity_pref;
...
delete from pfcomm_mig_job.faith_pref;
...
IF NEW.Area='Profiles'
THEN
update pfcomm_mig_job.account a
inner join pfcomm_test.Profiles p
on a.username=p.nickname 
...
-- Facebook links
insert into pfcomm_mig_job.acc_social_site (url,account_id,website_id) 
SELECT pp.Facebook,pmp.account_id,1 
FROM pfcomm_test.`Profiles` as pp, pfcomm_mig_job.profiles as pmp 
...
IF NEW.Area='memberships'
THEN

INSERT INTO pfcomm_mig_job.sys_acl_levels_members 
SELECT pp.account_id ,s.IDLevel ,s.DateStarts ,s.DateExpires ,s.TransactionID ,s.Upgrade 
FROM pfcomm_test.sys_acl_levels_members s 
INNER JOIN pfcomm_test.Profiles p 
ON s.idmember=p.id
INNER JOIN pfcomm_mig_job.profiles pp
ON p.nickname=pp.nickname
...
update pfcomm_mig_job.contactdetails c
inner join pfcomm_mig_job.account a
on c.account_id=a.account_id
inner join pfcomm_test.Profiles p
on a.username=p.nickname
...
ELSEIF NEW.UpdateYN=1 or NEW.DeleteYN=1 
...
DELETE FROM pfcomm_mig_job.account 
...
DELETE FROM pfcomm_mig_job.`profiles` 
where profile_id=(select profile_id from(select profile_id from pfcomm_mig_job.`profiles` 
				  where nickname=(select nickname from pfcomm_test.`Profiles` 
								  where id=NEW.ProfileID)) as p);
...
or p.nickname=concat(a.username,'(2)')
set p.account_id=a.account_id
where a.account_id=(select account_id from pfcomm_mig_job.account 
					where username=(select nickname from pfcomm_test.`Profiles` 
								  where id=NEW.ProfileID));
                                 
-- CHILD TABLE

DELETE FROM pfcomm_mig_job.child 
where Accounts_id=(select account_id from(select account_id from pfcomm_mig_job.account 
				   where username=(select nickname from pfcomm_test.`Profiles` 
								  where id=NEW.ProfileID)) as p);
...
DELETE FROM pfcomm_mig_job.aqb_pc_members_blocks 
where account_id=(select account_id from(select account_id from pfcomm_mig_job.account 
				  where username=(select nickname from pfcomm_test.`Profiles` 
								  where id=NEW.ProfileID)) as p);
...
DELETE FROM pfcomm_mig_job.aqb_pc_members_blocks 
where user_id=(select account_id from(select account_id from pfcomm_mig_job.account 
			   where username=(select nickname from pfcomm_test.`Profiles` 
							   where id=NEW.ProfileID)) as p);
...
DELETE FROM pfcomm_mig_job.bx_groups_main 
where author_id=(select account_id from(select account_id from pfcomm_mig_job.account 
			     where username=(select nickname from pfcomm_test.`Profiles` 
								 where id=NEW.ProfileID)) as p);
...
DELETE FROM pfcomm_mig_job.contactdetails 
where Account_id=(select account_id from(select account_id from pfcomm_mig_job.account 
				  where username=(select nickname from pfcomm_test.`Profiles` 
								  where id=NEW.ProfileID)) as p);
...
DELETE FROM pfcomm_mig_job.sys_albums 
where account_id=(select account_id from(select account_id from pfcomm_mig_job.account 
				  where username=(select nickname from pfcomm_test.`Profiles` 
								  where id=NEW.ProfileID)) as p);
...
DELETE FROM pfcomm_mig_job.rayvideofiles 
where account_id=(select account_id from(select account_id from pfcomm_mig_job.account 
				  where username=(select nickname from pfcomm_test.`Profiles` 
								  where id=NEW.ProfileID)) as p);
...
DELETE FROM pfcomm_mig_job.bx_photos_main 
where account_id=(select account_id from(select account_id from pfcomm_mig_job.account 
				  where username=(select nickname from pfcomm_test.`Profiles` 
								  where id=NEW.ProfileID)) as p);
...
DELETE FROM pfcomm_mig_job.letter 
where account_id=(select account_id from(select account_id from pfcomm_mig_job.account 
				  where username=(select nickname from pfcomm_test.`Profiles` 
								  where id=NEW.ProfileID)) as p);
...
DELETE FROM pfcomm_mig_job.birthfather_pref 
where account_id=(select account_id from(select account_id from pfcomm_mig_job.account 
				  where username=(select nickname from pfcomm_test.`Profiles` 
								  where id=NEW.ProfileID)) as p);
...
DELETE FROM pfcomm_mig_job.adoption_type_preference 
where account_id=(select account_id from(select account_id from pfcomm_mig_job.account 
				  where username=(select nickname from pfcomm_test.`Profiles` 
								  where id=NEW.ProfileID)) as p);
...
DELETE FROM pfcomm_mig_job.age_group_preference 
where account_id=(select account_id from(select account_id from pfcomm_mig_job.account 
				  where username=(select nickname from pfcomm_test.`Profiles` 
								  where id=NEW.ProfileID)) as p);
...
DELETE FROM pfcomm_mig_job.acc_social_site 
where account_id=(select account_id from(select account_id from pfcomm_mig_job.account 
				  where username=(select nickname from pfcomm_test.`Profiles` 
								  where id=NEW.ProfileID)) as p);
...
DELETE FROM pfcomm_mig_job.sys_acl_levels_members 
where account_id=(select account_id from(select account_id from pfcomm_mig_job.account 
				  where username=(select nickname from pfcomm_test.`Profiles` 
								  where id=NEW.ProfileID)) as p);
