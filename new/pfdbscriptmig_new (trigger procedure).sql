...
select * from pfcomm.`profiles` limit 10;
select * from pfcomm_api_mig.`profiles` limit 10;

insert into pfcomm.logtable(pid,updateddate,area)
select 77,now(),'fgjsdgfjsgjf'

select * from pfcomm_api_mig.`profiles` where profile_id=77
select * from pfcomm.`profiles` where id=2345

CREATE TABLE pfcomm.logtable
...
CREATE TRIGGER tr_logtable
AFTER INSERT ON pfcomm.logtable
FOR EACH ROW
    
UPDATE pfcomm_api_mig.`profiles`
SET publish_status = 777
WHERE profile_id=NEW.pid;
...
SELECT publishstatus as publish_status
,nickname
,now() as modified_at
...
UPDATE pfcomm_mig_new.`letter` as l
INNER JOIN pfcomm_new.`Profiles` as p 
ON l.pid=p.id and replace(l.plabel,'about_','')=p.defaultletter
...
SELECT p.account_id ,s.IDLevel ,s.DateStarts ,s.DateExpires ,s.TransactionID ,s.Upgrade 
FROM pfcomm_new.sys_acl_levels_members s 
inner join pfcomm_mig_new.profiles p on s.idmember=p.profile_id;
