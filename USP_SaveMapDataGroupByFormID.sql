...
formmaker_properties where formname='196_homestudyapplication_3'


formmaker_pages where form_id=3097
select library_field_name,name,* from formmaker_fields 
where page_id in (7403,7404) and library_field_name like '%add%'
...
Declare	@RunSql varchar(max),
	...
	@Sql NVARCHAR(max),@sqlParamDef nvarchar(Max),@SqlDataCheck NVARCHAR(max),
	@FormID INT,
	@User_Id INT,
	@ConnId INT,
	@ConnectionID INT,
...
WHERE  FS.FORM_ID = @FormID and  field_group_id<=0
...
SET @DataCount=0
	...
	SELECT @FormName = FormName FROM   #FormTbl WHERE Id = 1
	IF EXISTS(SELECT 1 FROM sys.Columns WHERE name ='isXML' AND object_id=Object_id(@FormName))
		BEGIN		
		SET @sqlParamDef	=   ' @DataCount INT output,@DataXML XML output'
		SET @SqlDataCheck	=   ' SELECT  @DataCount = 1,@DataXML=formFullData FROM '+ @FormName + 
								' WHERE ISNULL(isXML,0)=1 AND USER_ID = '+ CAST(@User_Id as varchar(20))+' AND CONNID = '+ CAST(@ConnId as varchar(20)) +' 
								  AND CONNECTIONID = ' +CAST(@ConnectionID as varchar(20))
		
		Exec sp_executesql @SqlDataCheck,@sqlParamDef,@DataCount = @DataCount output,@DataXML=@DataXML output

			IF @DataCount=1
			BEGIN
				DECLARE @idoc INT
				EXECUTE sp_xml_preparedocument @idoc OUTPUT, @DataXML
			
			
				   SELECT * into #tempData FROM OpenXML(@idoc,'/XMLFORMDATA/DATA', 1)		 

				   select t.localname as Field,tt.text as FieldValue INTO #tempFormData
				   from #tempData t
				   join #tempData tt on t.id = tt.parentid 
				   where t.nodetype = 1 and t.parentid >0
			 END
        END		
...
Begin Try
	 ...
	 IF @DataCount=1
	 BEGIN
			SELECT   @SaveText=FieldValue FROM #tempFormData WHERE  Field =@FormFieldName
	 END
	 ELSE
	 BEGIN
