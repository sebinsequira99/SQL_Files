...
Create Table #Tbl_agncyID (
 ...
 Begin
	 ...
	 SET @sqlqry = 'UPDATE '+ @TableName +' SET '+ @ColumnName +' =235 '
	 PRINT @TableName
	 Exec(@sqlqry)
	 SET @count=@count+1
 END
 ...
 --********C_ACCOUNT_KEY & AGENCY_ID update**************

--Disable trigger for user_agencies table
...
Select  235, user_id, agency_name, address_line_1, address_line_2, city, state, zip, country, website, phone, fax, email_id, birth_parent_number, adoptive_parent_number, after_hours_number, 
                         type_of_business, technology, no_of_years_in_business, no_of_staff, services, states_licensed_in, countries_licensed_in, no_of_adoptions_year, email_id_for_notifications, alert_preference, phone_to_sms, 
                         mision_statement_check, mision_statement_text, values_check, values_text, background_check, background_text, logo, nature_of_adoption, religions, payment_option, perfect_adoption_portal, other_info, 
                         specialties, 235, demo_video, agency_tax_id, county_id, doc_process_id, ConnId, theme_color, agency_description, system_description
From user_agencies
Set IDENTITY_INSERT user_agencies OFF

Select * From  user_agencies  
Delete from user_agencies Where agency_id = 190
Select * From  user_agencies  

--Enable trigger for user_agencies table